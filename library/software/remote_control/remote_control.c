...
/*   
遥控器上电后红绿灯模式判断方法：按下【up】键若小车无反应则为红灯模式，反之则是绿灯模式
*/
		...
		if(cur_ps2_mode == PSB_REDLIGHT_MODE)   			//	红灯模式下仅有遥杆有效
		{
		...
		else if (cur_ps2_mode == PSB_GREENLIGHT_MODE)  //绿灯模式下遥感与按键均有效
		{
			if (key_num != 0)
			{
				//前进后退控制
